...
**Implementation status:** [README.md](../../README.md) is the source of truth for shipped features. GitHub, Slack, and Gmail OAuth, credential refresh, connection-bound MCP sessions, and the local operator console are implemented. Gmail draft creation is the only supported write, with optional idempotency keys and uncertain-outcome handling. See [onboarding setup](../onboarding.md). Live provider interoperability remains unverified. REST tool execution, application-facing activity listing, the TypeScript helper, and broader deployment hardening remain future work. The implemented callback path is `/oauth/callback/{app_id}/{integration_id}`. This document describes the broader design target, not the complete current API.
