...
async def test_session_creation_does_not_need_a_provider_catalogue(rig):
...
async def test_rejects_removed_restrictions_and_malformed_idempotency_keys(rig):
    conn = await rig.connection()
    response = await rig.http.post(
        "/v1/sessions",
        headers=rig.headers(),
        json={"connection_ids": [conn["id"]], "restrictions": {"tools": ["issue_read"]}},
    )
        ...
        for key in ["short", "has spaces in it", 12345678]:
            with pytest.raises(MCPError) as error:
                await mcp.session.call_tool(
                    mcp.ravn_name("issue_read"), ARGS, meta={"ravn/idempotency-key": key}
                )
...
async def test_mcp_vertical_slice_and_call_status(rig, mode):
    ...
    async with rig.mcp(session, mode) as mcp:
        ...
        assert {t.name for t in tools.tools} == {
            ...
            mcp.ravn_name("list_issues"),
        }
        result = await mcp.call_tool(mcp.ravn_name("issue_read"), ARGS)
...
async def test_concurrent_alice_bob_are_isolated(rig):
    ...
    async def call(session):
        async with rig.mcp(session) as mcp:
            return (await mcp.call_tool(mcp.ravn_name("issue_read"), ARGS)).content[0].text
...
async def test_no_plaintext_credentials_or_results_in_database(rig):
    ...
    async with rig.mcp(session) as mcp:
        await mcp.call_tool(mcp.ravn_name("issue_read"), ARGS)
...
async def test_cross_user_access_rejected(rig, operation):
    ...
    if operation == "get":
        ...
        response = await rig.http.post(
            "/v1/sessions", headers=headers, json={"connection_ids": [conn["id"]]}
        )
...
async def test_writes_and_argument_escape_hatches_denied(rig, name, args):
    ...
    async with rig.mcp(session) as mcp:
        with pytest.raises(MCPError):
            await mcp.call_tool(mcp.ravn_name(name), args)
...
async def test_session_ttl_and_revocation(rig):
    ...
    for ttl in [59, 14401, True, "3600"]:
        response = await rig.http.post(
            "/v1/sessions",
            headers=rig.headers(),
            json={"connection_ids": [conn["id"]], "ttl_seconds": ttl},
        )
        assert response.status_code == 400
...
async def test_disconnect_stops_execution_not_history(rig):
    ...
    session = await rig.session(conn)
    async with rig.mcp(session) as mcp:
        call = await mcp.call_tool(mcp.ravn_name("issue_read"), ARGS)
    for _ in range(2):
        response = await rig.http.post(
...
async def test_revocation_during_discovery_prevents_dispatch(rig):
    ...
    async with rig.mcp(session) as mcp:
        ...
        pending = asyncio.create_task(mcp.call_tool(mcp.ravn_name("issue_read"), ARGS))
...
async def test_provider_schema_changes_are_used_without_local_pins(rig):
    session = await rig.session(await rig.connection())
    rig.fake.tools[0].input_schema = {"type": "object", "required": ["new_field"]}
    async with rig.mcp(session) as client:
        tools = (await client.list_tools()).tools
        assert (
            next(
                t for t in tools if t.name == client.ravn_name(rig.fake.tools[0].name)
            ).input_schema
            == rig.fake.tools[0].input_schema
        )
        with pytest.raises(MCPError) as error:
            await client.call_tool(client.ravn_name(rig.fake.tools[0].name), {})
        assert error.value.data["code"] == "invalid_arguments"
    assert not rig.fake.calls
...
async def test_import_and_validation_errors_never_echo_credentials(rig):
    ...
    for credential in [{"type": "bearer", "token": value}, {"type": "incorrect", "token": value}]:
        response = await rig.http.post(
            ...
            headers=rig.headers(),
            json={"integration_id": "records", "credential": credential},
        )
...
async def test_duplicate_headers_and_json_keys_rejected(rig):
    ...
    body = '{"integration_id":"records","integration_id":"evil"}'
    response = await rig.http.post(
        "/v1/connections/import",
        content=body,
